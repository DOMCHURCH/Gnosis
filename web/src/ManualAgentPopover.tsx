        ...
        <div style={{ padding: 11, display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <input autoFocus type="text" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") submit(); if (e.key === "Escape") props.onClose(); }}
              style={{ fontSize: 11, color: "#C9C9D6", background: "#171721", border: "2px solid #2C2C3E", padding: "6px 7px", outline: "none", fontFamily: MONO }} />
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {STATES.map((s) => (
                <button key={s} type="button" onClick={() => setFigState(s)}
                  style={{ flex: 1, fontFamily: MONO, fontSize: 9, letterSpacing: 1, textTransform: "uppercase", cursor: "pointer", padding: "6px 2px", background: figState === s ? "#22D3EE" : "#171721", color: figState === s ? "#0D0D12" : "#C9C9D6", border: `2px solid ${figState === s ? "#22D3EE" : "#2C2C3E"}` }}>
                  {s}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
            ...
            {props.mode === "edit" && (
              <button type="button" onClick={() => { props.onRemove(); props.onClose(); }} style={{ fontFamily: MONO, fontSize: 10, letterSpacing: 1, background: "#171721", color: "#F87171", border: "2px solid #2C2C3E", padding: "5px 10px", cursor: "pointer" }}>REMOVE</button>
            )}
            <button type="button" onClick={props.onClose} style={{ fontFamily: MONO, fontSize: 10, letterSpacing: 1, background: "#171721", color: "#6B6B7B", border: "2px solid #2C2C3E", padding: "5px 10px", cursor: "pointer" }}>✕</button>
          </div>
        </div>
      </div>
    </Overlay>
