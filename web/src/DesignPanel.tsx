...
export function DesignPanel(props: { shot: { path: string; before: string | null; after: string }; onClose: () => void }) {
  const { shot } = props;
  return (
    <div style={{ border: "2px solid #2C2C3E", background: "#121219", fontFamily: MONO }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 9px", borderBottom: "1px solid #2C2C3E" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: shot.before ? "1fr 1fr" : "1fr", gap: 1, background: "#2C2C3E" }}>
        {shot.before && (
