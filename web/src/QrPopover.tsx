...
function QrTile(props: { code: QrCode; compact: boolean }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 9, letterSpacing: 2, color: props.code.color, padding: "6px 11px", borderBottom: "2px solid #2C2C3E" }}>{props.code.title}</div>
 ...
 * existing dark palette (#0D0D12 bg, #2C2C3E border, no rounded corners, monospace).
 */
export function QrPopover(props: { title: string; codes: QrCode[]; onClose: () => void }) {
  ...
  return (
    <div onClick={props.onClose} style={{ position: "fixed", inset: 0, zIndex: Z.overlay, background: "rgba(5,5,8,0.72)", display: "flex", alignItems: "center", justifyContent: "center", padding: 12 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "#0D0D12", border: "2px solid #2C2C3E", fontFamily: MONO, width: many ? "min(660px, 96vw)" : "min(320px, 92vw)", maxHeight: "94vh", overflowY: "auto", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 11px", borderBottom: "2px solid #2C2C3E" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "stretch" }}>
          {props.codes.map((c, i) => (
            <div key={c.title} style={{ display: "flex", flex: "1 1 240px", minWidth: 0, borderLeft: i > 0 ? "2px solid #2C2C3E" : 0 }}>
