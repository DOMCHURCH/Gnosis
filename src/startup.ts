...
export async function boot(flags: Flags, cwd: string): Promise<Boot> {
  ...
  const engine = new Engine({ apiKey, cwd, systemPrompt, models, session, skills, fallbackModel: config.fallbackModel, autoCommit, maxSessionUsd: config.maxSessionUsd, workspaceRoots: config.workspaceRoots, routing: config.routing });
