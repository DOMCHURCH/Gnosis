...
function windowsFocusNotes(): string[] {
  return [
    ...
    "To LAUNCH something that is not running, use focus_window(action=\"launch\", app=\"...\"), then list to",
    "confirm it appeared, then focus it. A GUI app can take longer than the launch call waits, so if the",
    "warning says it started but no window appeared, WAIT and list again — do not go looking for it on screen.",
    "Never hunt the Start menu, the taskbar or a desktop icon by clicking at coordinates. You cannot see where",
    "those are, the layout differs on every machine, and each miss clicks something real and irreversible. If",
    "focus_window cannot bring an app up, say so and stop; do not fall back to clicking around the taskbar.",
    "Key names follow the X11 convention: `super` is the Windows key (NOT \"win\"), and it is `control`, `alt`,",
    "`shift`, `escape`, `return`, `delete`, `pageup`, `pagedown`. Combine with `+`, e.g. \"control+shift+escape\".",
  ];
}
...
export async function buildSystemPrompt(cwd: string, skills: LoadedSkill[] = [], mapTokens = 1024): Promise<string> {
  ...
  const lines = [
    ...
    "That rule has a hard limit attached: if TWO attempts at the same goal have not moved the screen toward it,",
    "stop and tell the user what you tried and what you saw. Do not try a third place to click. Repeatedly",
    "clicking at guessed coordinates is not progress, it is an irreversible random walk across whatever the user",
    "happens to have open, and it is worse than admitting the app did not come up.",
    "Prefer the non-GUI route whenever one exists. Opening a program, reading a file, checking whether something",
    "is running, changing a setting that has a CLI — do it with bash. The mouse is for things that genuinely",
    "have no other interface, not for work you already have a reliable tool for.",
    "",
