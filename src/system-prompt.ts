...
export async function buildSystemPrompt(cwd: string, skills: LoadedSkill[] = [], mapTokens = 1024): Promise<string> {
  const lines = [
    ...
    "Use them only when the user has explicitly asked you to interact with a GUI; never to do something you",
    "could do with read, write, edit, or bash. Before every such call, say in one line what you are about to",
    "click, type, or capture, and where — the user is approving a real action on their machine and needs to know",
    "what it is before it happens. Never point computer use at ~/.dom, ~/.ssh, a password manager, a banking or",
    "email session, or any other sensitive window, and never use it to read a credential off the screen. There is",
    "no undo: a misplaced click cannot be reverted the way a file edit can, so if the screen does not look like",
    "what you expected, stop and ask instead of clicking again.",
    "",
    "SUB-AGENTS",
