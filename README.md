...
A **building view** (toggle in the sidebar) renders the same event stream as a floor-based visualization: five floors — Coordinator, Planning, Coding, Application, Sub-agents — with each agent drawn as a plain-SVG figure on the floor matching its state (plan mode → Planning, an active edit → Coding, a running background job → Application, else Coordinator). Figures slide between floors on state change; a spawned sub-agent appears on the Sub-agents floor and leaves when it returns; a `send_message` between tabs draws a brief link; busy figures pulse and any awaiting approval show an amber marker. Clicking a figure opens a minichat for that agent without leaving the view. The floor logic lives in a plain module (`web/src/floors.js`) shared by the renderer and the test suite; no third-party sprite/tileset assets are used.

**Reconnect & responsive.** The server tags every forwarded event with a monotonic `seq` and keeps a ring buffer (last 2000). If the browser drops, it reconnects with backoff and passes `?since=<lastSeq>`; the server replays exactly what was missed — no gap, no duplicate, no snapshot resend — and only resyncs agent state with a fresh snapshot if the gap exceeds the buffer. The theme matches the terminal palette (`#0D0D12` background, `#22D3EE`→`#E879F9` accents), and under 900px the layout collapses to the sidebar+chat view (the building needs width).
