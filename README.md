...
An **office view** (toggle in the sidebar) renders the same event stream as a top-down 8px-grid office floor with five zones — Coordinator, Planning, Coding, Application, Sub-agents. Each agent is a pixel-art figure at a desk, placed by state: plan mode → Planning, ask/yolo → Coding, a tab coordinating sub-agents → Coordinator, one running a background job → Application, and `task()` sub-agents appear in Sub-agents. Figure state follows the bus — busy (animated) with the running tool as its action, amber *awaiting* on a permission request (with APPROVE/DENY that fire the real response), *speaking* just after a turn, else idle. Zones have fixed desk counts (coding 4, sub-agents 3, others 1); overflow agents go to the roster as off-floor rather than overlapping. Line events feed a chat rail; clicking an agent opens a detail panel whose STEER sends into that agent's input and DISMISS closes the tab. `window.domOffice` (add/update/think/remove/list/say/onUserMessage/onApproval) stays exposed as a debug overlay. The zone/state/placement logic lives in a plain module (`web/src/office.js`) shared by the renderer and the test suite; the design is a ported Claude Design mockup — plain SVG, no third-party sprite/tileset assets.

**Reconnect & responsive.** The server tags every forwarded event with a monotonic `seq` and keeps a ring buffer (last 2000). If the browser drops, it reconnects with backoff and passes `?since=<lastSeq>`; the server replays exactly what was missed — no gap, no duplicate, no snapshot resend — and only resyncs agent state with a fresh snapshot if the gap exceeds the buffer. The theme matches the terminal palette (`#0D0D12` background, `#22D3EE`→`#E879F9` accents), and under 900px the layout collapses to the sidebar+chat view (the office floor needs width).
